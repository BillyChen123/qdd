...
export const TASK_STATUS_VALUES = ['pending', 'running', 'blocked', 'completed'] as const;
export const TASK_PROMOTION_VALUES = ['pending', 'none', 'candidate-recorded', 'registered'] as const;
export const TASK_SKILLS_SECTION_COMMENT =
  '<!-- Each bullet must start with a skill ID. Optional description may follow after ":" or " - ". -->';

const SKILL_ID_WITH_OPTIONAL_DESCRIPTION_PATTERN = /^`?([A-Za-z0-9][A-Za-z0-9._/-]*)`?(?=\s*(?::|-\s+|$))/;

function renderTaskSkillLines(skillIds: string[]): string {
  return renderBulletList(skillIds, '- None specified.');
}

export function parseTaskSkillSection(body: string): { present: boolean; skillIds: string[] | null } {
...
export function extractTaskSkillIdsFromBody(body: string): string[] | null {
  const parsed = parseTaskSkillSection(body);
  return parsed.present ? parsed.skillIds : null;
}

export function renderTaskBody(record: TaskRecord, studyId: string, inputs: string[]): string {
  const inputLines =
    inputs.length > 0
...
export function renderTaskMarkdown(record: TaskRecord, studyId: string, inputs: string[]): string {
  return renderMarkdownDocument(record, renderTaskBody(record, studyId, inputs));
}

export function createExampleTaskRecord(): TaskRecord {
  return {
    task_id: 'TASK-001',
    study_id: 'STUDY-001',
...
export function createExampleTaskMarkdown(): string {
  ...
  const body = renderTaskBody(record, record.study_id, [
    ...
    '- singlecell/scrna/sc-batch-integration: use this task-level executor skill for first-pass batch correction and latent-space integration.'
  );
  return renderMarkdownDocument(record, body);
}

export const taskFileContract: ManagedFileContract = {
  id: 'task',
