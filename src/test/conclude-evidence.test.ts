...
import type { ConcludeEvidenceTableLocator, StudyRecord, TaskRecord } from '../types.js';
...
async function markRecordsCompleted(projectRoot: string, studyId: string, taskId: string): Promise<void> {
  const studyPath = `studies/${studyId}/study.md`;
  const study = await readMarkdownDocument<StudyRecord>(projectRoot, studyPath);
  ...
  const task = await readMarkdownDocument<TaskRecord>(projectRoot, taskPath);
...
function parseOracleSupport(value: string): { gene: string; log2fc: string; fdr: string } {
...
test('conclude builds a source-aware dossier with verifiable Parkinson claims and deduplicated assets', async () => {
  ...
  const locator = tableLocator as ConcludeEvidenceTableLocator;
  ...
  const json = JSON.parse(await fs.readFile(result.evidenceDossierJsonPath, 'utf-8')) as { schemaVersion: number; audit: { status: string } };
