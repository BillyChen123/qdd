...
async function createRenderingProject(): Promise<string> {
...
test('renderer refuses unresolved Gate 2 without creating final_paper', async () => {
  ...
  await assert.rejects(
    renderAcceptedStory({ projectRoot, storyPath: 'accepted-story/story.md', gate2Accepted: false, texCompiler: null }),
    /Gate 2 acceptance is required/
  );
...
test('committed accepted-story fixture renders through the focused CLI', async () => {
  ...
  const report = JSON.parse(stdout) as { pdf_status: string; checks: Record<string, string>; coverage: { ratio: number } };
