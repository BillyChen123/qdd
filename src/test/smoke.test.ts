...
test('qdd init records central domain skills source without projecting domain skills into project tool directories', async () => {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(domainSkillsSourceDir, 'singlecell', 'scrna', 'custom-qc', 'SKILL.md'),
      [
        '---',
    ...
    await fs.writeFile(
      path.join(domainSkillsSourceDir, 'singlecell', 'scrna', 'custom-qc', 'scripts', 'run.py'),
      'print("custom qc")\n',
      'utf-8'
    );
...
test('qdd status aggregates study/task frontmatter from the prototype layout', async () => {
  ...
  await fs.writeFile(
    path.join(projectRoot, 'boundaries.yaml'),
    [
      'boundaries:',
      '  - id: B001',
...
test('scatac executor skills expose coherent parameter contracts and entry scripts', async () => {
  ...
  for (const relativeRoot of skillRoots) {
    ...
    const parameters = parseYaml<{
      entry_script?: string;
      output_contract?: { main_h5ad?: string; report?: string; result?: string };
    }>(await fs.readFile(path.join(skillRoot, 'parameters.yaml'), 'utf-8'));
    ...
    const entryScriptPath = path.join(skillRoot, parameters.entry_script as string);
...
test('cellxgene public-data executor skill exposes coherent parameter contract and entry script', async () => {
  ...
  const parameters = parseYaml<{
    entry_script?: string;
    output_contract?: { search?: object; download?: object };
  }>(await fs.readFile(path.join(skillRoot, 'parameters.yaml'), 'utf-8'));
...
test('qdd instructions returns project, study, and task guidance for existing prototype records', async () => {
  ...
  assert.ok(
    studyCloseInstructions.rules.includes(
      'Apply studies/STUDY-001/output/boundary-updates.yaml through qdd boundaries apply --file studies/STUDY-001/output/boundary-updates.yaml before running qdd close-study.'
    )
  );
  ...
  assert.ok(
    studyCloseInstructions.rules.includes(
      'If this study introduced reusable downloaded datasets under artifacts/data/, record their stable source, alias, and intended reuse role in context/resources.md before closure.'
    )
  );
  ...
  assert.ok(
    studyExploreInstructions.rules.includes(
      'Use qdd boundaries score --targets <B001,B002> --json or qdd boundaries score --study <id> --json to test legality, readiness, and frontier breadth before finalizing or reshaping a study.'
    )
  );
  assert.ok(
    studyExploreInstructions.rules.includes(
      "Distinguish the user's long-range scientific target from the current executable study; if active ancestors remain, preserve the long-range target in explanation but downshift the current study to the suggested frontier."
    )
  );
  assert.ok(
    studyExploreInstructions.rules.includes(
      'Do not hide cross-layer study scope by merely adding more tasks; use boundary decomposition first, then write the small first-pass task graph for the resulting current study.'
    )
  );
  assert.ok(
    studyExploreInstructions.rules.includes(
  ...
  assert.ok(
    studyExploreInstructions.rules.includes(
      'Record explicit target_boundaries in study.md frontmatter and in the ## Target Boundaries section.'
    )
  );

  await fs.writeFile(
    path.join(projectRoot, 'studies', 'STUDY-001', 'output', 'public_data_request.yaml'),
    [
      'source: cellxgene',
      'modality: scrna',
      'goal: validation',
  ...
  assert.ok(
    studyApplyPublicDataInstructions.rules.includes(
      'Treat studies/STUDY-001/output/public_data_request.yaml as the planning-owned handoff for public-data selection.'
    )
  );
  assert.ok(
    studyApplyPublicDataInstructions.rules.includes(
      'Planning may narrow public-data candidates, but apply may only consume the selected targets already written there.'
    )
  );
  ...
  assert.ok(
    studyClosePublicDataInstructions.rules.includes(
      'If the selected public datasets were downloaded successfully, qdd-close should decide whether they belong in carried-forward project resources and document them explicitly in context/resources.md.'
    )
  );
  assert.ok(
    studyExploreInstructions.rules.includes(
...
test('qdd boundaries can apply project-local updates and render a project-local HTML report', async () => {
  ...
  await fs.writeFile(
    path.join(projectRoot, '.qdd', 'seed-boundaries.yaml'),
    [
      'updates:',
      '  - action: add',
      '    boundary:',
  ...
  assert.deepEqual(
    boundaryState.boundaries.map((boundary) => boundary.id),
    ['B001', 'B002']
  );
  ...
  await fs.writeFile(
    path.join(projectRoot, 'studies', createdStudy.studyId, 'output', 'boundary-updates.yaml'),
    ['updates:', '  - action: resolve', '    id: B001', ''].join('\n'),
    'utf-8'
  );
...
test('qdd closeStudy promotes artifacts before writing question_delta', async () => {
  ...
  await fs.writeFile(
    path.join(projectRoot, 'studies', createdStudy.studyId, 'output', 'artifact-candidates.yaml'),
    [
      'artifact_candidates:',
...
test('qdd validate warns on placeholder onboarding state and reports broken links / missing local skills', async () => {
  ...
  await fs.writeFile(
    path.join(projectRoot, 'studies', 'STUDY-001', 'output', 'boundary-updates.yaml'),
    ['updates:', '  - action: invalid', '    id: B999', ''].join('\n'),
    'utf-8'
  );
