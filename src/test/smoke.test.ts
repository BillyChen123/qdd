...
async function recordDossierBackedClaimFixture(
  projectRoot: string,
  studyId: string,
  taskId: string,
  stem: string
...
test('conclude tolerates non-string study summary fields during evidence harvest', async () => {
  ...
  await setTaskState(
    ...
    task.taskId,
    'completed',
    'The bounded signal is associated with the candidate state and remains non-mechanistic.'
  );
  await fs.writeFile(
  ...
  await writeMarkdownDocument(
    ...
    {
      ...document.frontmatter,
      expected_artifacts: ['One markdown summary', 42, { kind: 'table', status: 'draft' }] as unknown as string[],
    },
    document.body
  );
...
test('qdd conclude CLI accepts selected story input and returns planning artifacts in json', async () => {
  ...
  await fs.writeFile(
    selectedStoryPath,
    `${structuredSelection.slice(0, frontmatterEnd)}\n\n# Misleading Prose\n\nSelected Story ID: story-999\n`,
    'utf-8'
  );
  ...
  const parsed = JSON.parse(stdout) as {
    ...
    finalPaperArtifacts: null | {
      ...
      referencesBib: { status: string };
      citationIntegrity: { status: string };
      pdf: { status: string };
      word: { status: string };
    };
...
test('runConclude keeps selection gate without final paper and writes final package after selected story', async () => {
  ...
  await setTaskState(
    ...
    discoveryTask.taskId,
    'completed',
    'The selected cohort signal is associated with the candidate state, while mechanistic intervention remains unavailable.'
  );
  await fs.writeFile(
    path.join(projectRoot, 'context', 'memory', `${discoveryStudy.studyId}.md`),
