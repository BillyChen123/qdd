...
export function probeTexCompiler(): TexCompiler | null {
  const latexmk = findExecutable('latexmk');
  const bibtex = findExecutable('bibtex');
  if (latexmk && bibtex) return { kind: 'latexmk', command: latexmk };
  ...
  if (pdflatex && bibtex) return { kind: 'pdflatex', command: pdflatex, bibtexCommand: bibtex };
  return null;
}

function requireMatch(value: string, pattern: RegExp, description: string): void {
  if (!pattern.test(value)) throw new Error(`Manuscript validation failed: ${description}.`);
}

function parseKeys(tex: string): string[] {
  return [...tex.matchAll(/\\cite(?:[A-Za-z*]*)?(?:\[[^\]]*\]){0,2}\{([^}]+)\}/g)]
    .flatMap((match) => match[1]!.split(',').map((key) => key.trim()).filter(Boolean));
}

function parseBibtexKeys(bibtex: string): string[] {
  const keys = [...bibtex.matchAll(/^\s*@\w+\s*\{\s*([^,\s]+)\s*,/gmi)].map((match) => match[1]!);
  if (keys.length === 0) throw new Error('Manuscript validation failed: references.bib has no BibTeX entries.');
  const duplicates = keys.filter((key, index) => keys.indexOf(key) !== index);
  if (duplicates.length > 0) throw new Error(`Manuscript validation failed: duplicate BibTeX keys: ${[...new Set(duplicates)].join(', ')}.`);
  return keys;
}

function parseEnvironments(tex: string, environment: 'figure' | 'table'): string[] {
  return [...tex.matchAll(new RegExp(`\\\\begin\\{${environment}\\}`, 'g'))].map((match) => match[0]);
}

function validateBalancedEnvironments(tex: string): void {
  const stack: string[] = [];
  for (const match of tex.matchAll(/\\(begin|end)\{([A-Za-z*]+)\}/g)) {
    const kind = match[1]!;
    const environment = match[2]!;
    if (kind === 'begin') stack.push(environment);
    else if (stack.pop() !== environment) throw new Error(`Manuscript validation failed: unbalanced ${environment} environment.`);
  }
  if (stack.length > 0) throw new Error(`Manuscript validation failed: unclosed ${stack.at(-1)} environment.`);
}

async function compilePdf(packageDir: string, compiler: TexCompiler): Promise<void> {
...
export async function validateManuscriptPackage(packagePath: string, texCompiler: TexCompiler | null | undefined = undefined): Promise<ManuscriptPackageReport> {
  const packageDir = path.resolve(packagePath);
  for (const requiredFile of REQUIRED_FILES) await fs.access(path.join(packageDir, requiredFile));
  ...
  const firstFloatIndex = Math.min(
    ...['\\begin{figure}', '\\begin{table}'].map((marker) => {
  ...
  if (missingCitations.length > 0) throw new Error(`Manuscript validation failed: citations missing from references.bib: ${missingCitations.join(', ')}.`);
  ...
  if (duplicateLabels.length > 0) throw new Error(`Manuscript validation failed: duplicate labels: ${[...new Set(duplicateLabels)].join(', ')}.`);
  ...
  if (missingReferences.length > 0) throw new Error(`Manuscript validation failed: broken cross-references: ${[...new Set(missingReferences)].join(', ')}.`);
  const graphics = [...tex.matchAll(/\\includegraphics(?:\[[^\]]*\])?\{([^}]+)\}/g)].map((match) => match[1]!);
  for (const graphic of graphics) {
    if (!graphic.startsWith('figures/')) throw new Error(`Manuscript validation failed: figure asset must be under figures/: ${graphic}.`);
    await fs.access(path.join(packageDir, graphic));
  }

  const compiler = texCompiler === undefined ? probeTexCompiler() : texCompiler;
  if (compiler) await compilePdf(packageDir, compiler);
  const report: ManuscriptPackageReport = {
    schema_version: 1,
...
export function natureTemplateRoot(): string {
