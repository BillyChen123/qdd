...
import type {
  ...
  ArtifactIndex,
  ArtifactIndexEntry,
  ConcludeEvidenceCategory,
  ConcludeEvidenceClue,
  ConcludeEvidenceItem,
  ConcludeEvidenceSignal,
  ConcludeHarvestSummary,
  ConcludePreflightResult,
  ConcludeRenderingTool,
  ConcludeRenderingToolStatus,
  ConcludeStudyPreflight,
  EvolutionState,
  RunConcludeOptions,
  RunConcludeResult,
  StudyRecord,
...
import {
  CONCLUDE_EVIDENCE_KIND_VALUES,
  CONCLUDE_EVIDENCE_SIGNAL_VALUES,
  CONCLUDE_RENDERING_TOOL_VALUES,
} from '../types.js';
...
const SIGNAL_ORDER: Record<ConcludeEvidenceSignal, number> = {
  positive: 0,
  negative: 1,
  blocked: 2,
  downgraded: 3,
...
const STUDY_OUTPUT_CATEGORY_BY_SUBDIR = {
  ...
  tables: 'table',
  reports: 'report',
} as const satisfies Record<string, ConcludeEvidenceCategory>;

interface ParsedStudyMemory {
  studyId: string;
  relativePath: string;
  question: string | null;
  outcomeKind: string | null;
  summary: string | null;
  resolvedBoundaries: string[];
  openBoundaries: string[];
}

interface RawEvidenceItem extends Omit<ConcludeEvidenceItem, 'id'> {}
interface RawEvidenceClue extends Omit<ConcludeEvidenceClue, 'id'> {}
...
function extractMarkdownSectionText(body: string, heading: string): string | null {
  const pattern = new RegExp(`## ${escapeRegExp(heading)}\\n\\n([\\s\\S]*?)(?=\\n## |$)`);
  const match = body.match(pattern);
  return match ? match[1].trim() || null : null;
}

function parseStudyMemory(relativePath: string, body: string): ParsedStudyMemory {
...
function parseProducedBy(producedBy: string): { studyId: string | null; taskId: string | null } {
...
function buildDefaultRunId(now = new Date()): string {
  return now.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}Z$/, 'Z');
}

function buildStudyOutputPath(studyId: string, subdir: keyof typeof STUDY_OUTPUT_CATEGORY_BY_SUBDIR): string {
...
async function collectReusableContextPaths(projectRoot: string, relativeDir: string = PATHS.contextDir): Promise<string[]> {
  const absoluteDir = path.join(projectRoot, relativeDir);
  if (!(await FileSystemUtils.directoryExists(absoluteDir))) {
    return [];
  }
...
async function resolveExecutableOnPath(binary: ConcludeRenderingTool): Promise<ConcludeRenderingToolStatus> {
...
async function buildPreflight(projectRoot: string, studies: StudyRecord[]): Promise<ConcludePreflightResult> {
  const memoryPaths = await listStudyMemoryPaths(projectRoot);
  const memoryByStudyId = new Map(
    memoryPaths
      .map((relativePath) => {
  ...
  const studyChecks: ConcludeStudyPreflight[] = [];
  ...
  for (const study of [...studies].sort((left, right) => left.study_id.localeCompare(right.study_id))) {
    ...
    const invalidCandidatePaths = (await inspectArtifactCandidatePaths(projectRoot, study.study_id)).map(
      (issue) => `${issue.path || `#${issue.index}`}: ${issue.reason}`
    );
...
function detectSignalsInText(text: string): ConcludeEvidenceSignal[] {
  const normalized = text.trim().toLowerCase();
  if (!normalized) {
    return [];
  }

  const detected = new Set<ConcludeEvidenceSignal>();
  const patterns: Array<[ConcludeEvidenceSignal, RegExp[]]> = [
...
function pushClue(target: RawEvidenceClue[], clue: RawEvidenceClue | null | undefined): void {
...
function pushTextSignals(
  target: RawEvidenceClue[],
  text: string | null | undefined,
  options: Omit<RawEvidenceClue, 'signal' | 'text'>
): void {
  if (!text?.trim()) {
    return;
  }
...
function createInitialKindCounts(): Record<(typeof CONCLUDE_EVIDENCE_KIND_VALUES)[number], number> {
  return {
    'promoted-artifact': 0,
    'study-output': 0,
    'study-memory': 0,
...
function createInitialSignalCounts(): Record<(typeof CONCLUDE_EVIDENCE_SIGNAL_VALUES)[number], number> {
  return {
    positive: 0,
    negative: 0,
    blocked: 0,
...
function summarizeEvidence(items: ConcludeEvidenceItem[], clues: ConcludeEvidenceClue[]): ConcludeHarvestSummary {
...
function dedupeEvidenceItems(items: RawEvidenceItem[]): ConcludeEvidenceItem[] {
  const seen = new Map<string, RawEvidenceItem>();
...
function dedupeEvidenceClues(clues: RawEvidenceClue[]): ConcludeEvidenceClue[] {
  const seen = new Map<string, RawEvidenceClue>();
...
async function resolveSymlinkArtifactPath(projectRoot: string, relativePath: string): Promise<string | null> {
...
function buildArtifactEvidenceItem(entry: ArtifactIndexEntry): RawEvidenceItem {
...
async function buildStudyOutputEvidenceItems(
  projectRoot: string,
  studyId: string,
  candidateByPath: Map<string, ArtifactCandidateEntry>,
  ...
  for (const [subdir, category] of Object.entries(STUDY_OUTPUT_CATEGORY_BY_SUBDIR)) {
    const outputPaths = await listFilesRecursively(projectRoot, buildStudyOutputPath(studyId, subdir as keyof typeof STUDY_OUTPUT_CATEGORY_BY_SUBDIR));
...
async function collectEvidence(projectRoot: string, preflight: ConcludePreflightResult): Promise<{
  evidenceItems: ConcludeEvidenceItem[];
  evidenceClues: ConcludeEvidenceClue[];
}> {
  ...
  const rawClues: RawEvidenceClue[] = [];
  ...
  for (const study of studies) {
    ...
    const candidateByPath = new Map<string, ArtifactCandidateEntry>();
  ...
  for (const task of tasks) {
    ...
    const document = await readMarkdownDocument<TaskRecord>(projectRoot, taskPath);
...
function renderToolStatus(tool: ConcludeRenderingToolStatus): string {
  return `${tool.tool}: ${tool.available ? `AVAILABLE (${tool.resolved_path})` : 'MISSING'}`;
}

function renderStudyCheck(check: ConcludeStudyPreflight): string[] {
  return [
    `### ${check.study_id}`,
    '',
...
function renderEvidenceItem(item: ConcludeEvidenceItem): string[] {
  return [
    `### ${item.id} ${item.title}`,
    '',
    `- Kind: ${item.kind}`,
...
function buildEvidenceAuditMarkdown(result: RunConcludeResult): string {
  const lines: string[] = [
    '# Evidence Audit',
    '',
...
function resolveOutputDir(projectRoot: string, outputDir: string | undefined, runId: string): string {
...
export async function runConclude(projectRoot: string, options: RunConcludeOptions = {}): Promise<RunConcludeResult> {
  ...
  const result: RunConcludeResult = {
    run_id: runId,
