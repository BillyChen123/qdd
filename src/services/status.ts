...
import { inspectArtifactCandidatePaths, listNonCanonicalStudyOutputEntries } from '../runtime/evidence.js';
import { listRecentStudyMemoryPaths, readEvolutionState, summarizeEvolutionBoundaries, getCurrentProjectQuestion } from '../runtime/evolution.js';
import { readYamlFile } from '../runtime/store.js';
...
export async function buildStatus(projectRoot: string): Promise<StatusJson> {
  ...
  const studiesWithUnpackagedOutput = (
    await Promise.all(
  ...
  return {
    ...
    studies: {
      active: studies
        .filter((study) => {
          const state = deriveStudyLifecycleState(
            study,
            tasks.filter((task) => task.study_id === study.study_id || (study.task_ids ?? []).includes(task.task_id))
          );
