...
import {
  describeArtifactCandidateManifestShapeIssue,
...
function allowedPlanningCategoriesForRole(roleName: string): PlanningOnlySkillCategory[] {
...
function validateStudyTargetBoundaries(study: StudyRecord, knownBoundaryIds: Set<string>, issues: ValidationIssue[]): void {
  const studyPath = `${PATHS.studiesDir}/${study.study_id}/study.md`;
...
function validateArtifactCandidateManifest(studyId: string, manifest: ArtifactCandidateManifest, issues: ValidationIssue[]): void {
...
async function collectContextPaths(projectRoot: string, relativeDir: string = PATHS.contextDir): Promise<string[]> {
...
export async function listContext(projectRoot: string): Promise<ContextJson> {
  ...
  for (const relativePath of contextPaths) {
    const data = relativePath.endsWith('.md')
      ...
      : await readYamlFile<unknown>(projectRoot, relativePath);
    contextEntries.push({
      path: relativePath,
...
export async function validateProject(projectRoot: string): Promise<ValidationResult> {
  ...
  let knownBoundaryIds = new Set<string>();
