import type {
  ConcludeCanonicalStory,
  ConcludeClaimStrength,
  ConcludeEvidenceDossier,
  ConcludeEvidenceDossierUnit,
  ConcludeEvidenceItem,
  ConcludeManuscriptClaimGraph,
  ConcludeQuestionEvolutionTransition,
  ConcludeSemanticStoryBeatProposal,
  ConcludeSemanticStoryPlanner,
  ConcludeSemanticStoryPlannerInput,
  ConcludeSemanticStoryProposal,
  ConcludeStoryEvidenceRole,
  ConcludeStoryEvidenceRoleAssignment,
  ConcludeStoryFigureTablePlanEntry,
  ConcludeStoryOmissionEntry,
  ConcludeStoryOracleConstraints,
  ConcludeStoryPlan,
  ConcludeStoryPlanAuditViolation,
  ConcludeStoryResultsBeat,
...
interface DefaultStage {
  key: 'state' | 'resolution' | 'candidate' | 'global' | 'pathway' | 'event';
  question: string;
  answer: string;
  boundedInterpretation: string;
  transition: Exclude<ConcludeSemanticStoryBeatProposal['transition'], 'closes'>;
...
function unitStudyIds(unit: ConcludeEvidenceDossierUnit): string[] {
  return uniqueStrings(unit.provenance.sources.flatMap((source) => source.studyIds));
}

function isSafeNarrativeUnit(unit: ConcludeEvidenceDossierUnit): boolean {
...
function unitScore(unit: ConcludeEvidenceDossierUnit): number {
  const statement = claimStatement(unit);
...
function findExactUnit(
  units: ConcludeEvidenceDossierUnit[],
  pattern: RegExp,
  preferredStudyIds: string[] = []
): ConcludeEvidenceDossierUnit | null {
  return selectUnits(units, pattern, 1, preferredStudyIds)[0] ?? null;
}

export function buildConcludeQuestionEvolutionTransitions(
  evolution: EvolutionState | null | undefined
): ConcludeQuestionEvolutionTransition[] {
  if (!evolution) {
    return [];
  }
...
function proposalBeatIsValid(value: unknown): value is ConcludeSemanticStoryBeatProposal {
  if (!value || typeof value !== 'object') {
    return false;
  }
  const beat = value as Partial<ConcludeSemanticStoryBeatProposal>;
  const evidence = beat.evidence as Partial<ConcludeSemanticStoryBeatProposal['evidence']> | undefined;
...
function isSemanticStoryProposal(value: unknown): value is ConcludeSemanticStoryProposal {
  if (!value || typeof value !== 'object') {
    return false;
  }
  const proposal = value as Partial<ConcludeSemanticStoryProposal>;
...
function stageAssetIds(units: ConcludeEvidenceDossierUnit[]): string[] {
  return uniqueStrings(units.flatMap((unit) => unit.assetCandidateIds));
}

function transitionIdsForUnits(
  units: ConcludeEvidenceDossierUnit[],
  transitions: ConcludeQuestionEvolutionTransition[]
): string[] {
...
function evolutionStudyIdsMatching(
  input: ConcludeSemanticStoryPlannerInput,
  pattern: RegExp
): string[] {
...
function buildDefaultStages(input: ConcludeSemanticStoryPlannerInput): DefaultStage[] {
  ...
  let stateUnits = [qkiState, celf2State].filter((unit): unit is ConcludeEvidenceDossierUnit => Boolean(unit));
  ...
  const resolutionUnit = findExactUnit(
    boundaries,
    /cannot determine which isoforms|requires transcript-level resolution|3[' -]?prime.*(?:isoform|transcript)|cannot provide isoform-level|full transcript.*limitation/i,
    resolutionStudyIds
  );
  if (resolutionUnit) {
    const resolutionAnchor = findExactUnit(
      results,
      /QKI.*(?:mean|expression).*(?:astrocyte|n\s*a=|subcluster)/i,
      resolutionStudyIds
    );
    stages.push({
      key: 'resolution',
  ...
  const globalUnits = [globalSummary, stableAnchor].filter((unit): unit is ConcludeEvidenceDossierUnit => Boolean(unit));
  ...
  const eventUnits = [localizedCount, boundaryRemodeling].filter((unit): unit is ConcludeEvidenceDossierUnit => Boolean(unit));
...
function uniqueUnits(units: ConcludeEvidenceDossierUnit[]): ConcludeEvidenceDossierUnit[] {
...
function buildDefaultProposal(input: ConcludeSemanticStoryPlannerInput): ConcludeSemanticStoryProposal | null {
  ...
  const causalBoundary = findExactUnit(
    safeBoundaries,
    /does not support.*(?:broad|driver|causal)|cannot establish.*(?:causal|specific)|not establish.*mechanistic|plausible bridge.*not core-defining/i,
    closureStudyIds
  ) ?? safeBoundaries.find((unit) => !UNSAFE_CAUSAL_ASSERTION_PATTERN.test(claimStatement(unit))) ?? null;
  if (stages.length === 0 || !causalBoundary) {
    return null;
  }

  const motifValidation = findExactUnit(
    input.dossier.evidenceUnits.filter((unit) => unit.eligibility === 'results' && isSafeNarrativeUnit(unit)),
    /(?:QKI|CELF2).*confidence score|motif-compatible|binding-context-compatible/i,
    closureStudyIds
  );
  ...
  const mainBeatUnits: ConcludeEvidenceDossierUnit[][] = [];
  const beats: ConcludeSemanticStoryBeatProposal[] = stages.map((stage) => {
  ...
  const roleByClaimId = new Map<string, ConcludeStoryEvidenceRole>();
  const beatSequencesByClaimId = new Map<string, number[]>();
  for (const [index, beat] of beats.entries()) {
    const roles: Array<[ConcludeStoryEvidenceRole, string[]]> = [
  ...
  const omissions: ConcludeStoryOmissionEntry[] = [];
  for (const unit of input.dossier.evidenceUnits) {
    if (includedIds.has(unit.id)) {
      continue;
    }
    const statement = claimStatement(unit);
    let role: Extract<ConcludeStoryEvidenceRole, 'context' | 'supplementary' | 'excluded'>;
    let category: ConcludeStoryOmissionEntry['category'];
  ...
  const parkinsonShape = ['state', 'candidate', 'global', 'pathway', 'event'].every((key) => stageKeys.has(key as DefaultStage['key']));
...
export const defaultConcludeSemanticStoryPlanner: ConcludeSemanticStoryPlanner = {
  plan(input) {
    return buildDefaultProposal(input);
  },
};

function roleAssignmentMap(
  proposal: ConcludeSemanticStoryProposal,
  beats: ConcludeStoryResultsBeat[]
): ConcludeStoryEvidenceRoleAssignment[] {
...
function buildCanonicalStory(
  ...
  const transitionById = new Map(input.questionEvolutionTransitions.map((transition) => [transition.id, transition] as const));
  const unitById = new Map(input.dossier.evidenceUnits.map((unit) => [unit.id, unit] as const));
  const assetById = new Map(input.dossier.assetCandidates.map((asset) => [asset.id, asset] as const));
  const beats: ConcludeStoryResultsBeat[] = proposal.beats.map((beat, index) => {
  ...
  const assignmentByClaimId = new Map(assignments.map((assignment) => [assignment.claimId, assignment] as const));
  ...
  const graphNodes = mainAssignments.map((assignment) => {
    ...
    return {
      claimId: assignment.claimId,
      role: assignment.role as 'core' | 'bridge' | 'validation' | 'boundary',
      statement: unit ? sanitizeReaderText(claimStatement(unit)) : '',
      claimStrength: unit?.narrative.claimStrength ?? 'bounded' as ConcludeClaimStrength,
  ...
  const graphEdges: ConcludeManuscriptClaimGraph['edges'] = [];
  for (let index = 0; index < beats.length - 1; index += 1) {
  ...
  const figureTableSequence: ConcludeStoryFigureTablePlanEntry[] = [];
  ...
  const claimGraph: ConcludeManuscriptClaimGraph = {
    schemaVersion: 1,
  ...
  return {
    ...
    resultsArc: beats.map((beat, index) => ({
      ...
      role: (index === 0 ? 'lead' : 'supporting') as 'lead' | 'supporting',
...
function buildViabilityDiagnostics(
  beats: ConcludeStoryResultsBeat[],
  assignments: ConcludeStoryEvidenceRoleAssignment[],
...
function collectReaderVisibleFields(story: ConcludeCanonicalStory): Array<{ field: string; value: string; claimIds: string[] }> {
  return [
...
export function auditConcludeStoryPlan(
  story: ConcludeCanonicalStory | null,
  input?: ConcludeSemanticStoryPlannerInput
): ConcludeStoryPlan['audit'] {
  ...
  const unitById = new Map(input?.dossier.evidenceUnits.map((unit) => [unit.id, unit] as const) ?? []);
  const assetById = new Map(input?.dossier.assetCandidates.map((asset) => [asset.id, asset] as const) ?? []);
  const safetyByClaimId = new Map(input?.claimSafety.map((safety) => [safety.claimId, safety] as const) ?? []);
  ...
  const assignmentByClaimId = new Map<string, ConcludeStoryEvidenceRoleAssignment>();
  ...
  for (const beat of story.resultsBeats) {
    const roleClaims: Array<[ConcludeStoryEvidenceRole, string[]]> = [
      ['core', beat.evidence.coreClaimIds],
      ['bridge', beat.evidence.bridgeClaimIds],
      ['validation', beat.evidence.validationClaimIds],
      ['boundary', beat.evidence.boundaryClaimIds],
  ...
  const omissionByClaimId = new Map(story.omissionLedger.map((entry) => [entry.claimId, entry] as const));
...
export async function buildConcludeStoryPlan(
  dossier: ConcludeEvidenceDossier,
  options: StoryPlanBuildOptions = {}
): Promise<ConcludeStoryPlan> {
  const input: ConcludeSemanticStoryPlannerInput = {
    schemaVersion: 1,
    dossier,
  ...
  try {
    ...
    return {
      ...
      diagnostics: [`Semantic story planning failed: ${(error as Error).message}`],
      story: null,
      audit: {
        status: 'fail',
        violations: [{
