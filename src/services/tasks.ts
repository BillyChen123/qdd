...
import type { AddTaskOptions, CreatedTaskResult, TaskRecord, StudyRecord } from '../types.js';
...
export async function findTaskDocument(
  projectRoot: string,
  taskId: string
): Promise<{ studyId: string; relativePath: string; record: TaskRecord; body: string }> {
