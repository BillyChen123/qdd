...
When running in auto mode, after you have completed all close-mode work above — wrote the question_delta into `evolution.yaml`, promoted artifacts, wrote study memory, refreshed `research-map.html`, and cleaned scratch leftovers:

### Step 1: Read Termination Signals

Read `evolution.yaml` and inspect the latest study event you just wrote. Evaluate these conditions:

| Condition | Meaning |
|-----------|---------|
| `change_type` = `confirmation` | The research question has been sufficiently answered |
| `change_type` = `dissolution` | The question is undecidable within current resource boundaries |
| `open_boundaries` is empty (`[]`) | No remaining uncertainties — the research frontier is closed |
| `next_candidates` is empty (`[]`) | No credible follow-up direction exists |

### Step 2: Decide — Terminate or Continue

**If ANY of the four conditions above is true, TERMINATE the fork chain:**

1. Output a concise research summary covering:
   - The original research question from `contract.yaml`
   - How many studies were completed
   - The final question state and change type
   - Remaining open boundaries (if any)
   - Key artifacts produced
2. Do NOT fork another agent. The research loop is complete.

**If NONE of the conditions is true, CONTINUE the fork chain:**

1. Compute the next study ID:
   - Read `evolution.yaml` to find the highest STUDY-NNN ID
   - Increment to the next number (e.g., STUDY-002 → STUDY-003)
2. Fork a Study Brain sub-agent for the next cycle:

   ```
   /fork Study Brain for <next STUDY> with command qdd-propose.

   Context to pass:
   - The project root directory: <current working directory>
   - Run: qdd instructions <next STUDY> --command qdd-propose --json
   - Read the bootstrap prompt at: src/runtime/bootstrap-prompts/qdd-propose.md
   - Follow the qdd-propose workflow normally.
   - Read evolution.yaml for next_candidates before planning.
   - The next_candidates from the previous close are: <list from evolution.yaml>
   - At the end of your work, follow the "Auto Mode: Fork Next Agent" section in your instructions.
   ```

3. Your work is done. The fork chain continues from here.

### Step 3: Report the Decision

Whether terminating or continuing, report clearly:
- Which study was just closed
- What the `change_type` was and why
- What `open_boundaries` remain
- What `next_candidates` were recorded
- Whether the chain terminates or continues to the next study
