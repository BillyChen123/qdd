...
import type { StudyRecord, TaskRecord } from '../types.js';
...
async function readFrontmatterIfExists<T>(filePath: string): Promise<T | null> {
  try {
    const content = await fs.readFile(filePath, 'utf-8');
...
export async function discoverStudies(projectRoot: string): Promise<StudyRecord[]> {
    ...
    const studies = await Promise.all(
      entries
        ...
        .map(async (entry) => {
          ...
          return await readFrontmatterIfExists<StudyRecord>(studyPath);
        })
    );
    return studies.filter((study): study is StudyRecord => study !== null);
  } catch (error: any) {
    if (error.code === 'ENOENT') {
      return [];
    }
...
export async function discoverTasks(projectRoot: string): Promise<TaskRecord[]> {
  const studiesDir = path.join(projectRoot, 'studies');
  ...
  try {
    ...
    for (const studyEntry of studyEntries) {
      ...
      for (const taskEntry of taskEntries) {
        ...
        const task = await readFrontmatterIfExists<TaskRecord>(taskPath);
        if (task) {
          tasks.push({
            ...task,
