...
export async function readBoundaryState(projectRoot: string): Promise<BoundaryState> {
  ...
  const state = await readYamlFile<BoundaryState>(projectRoot, PATHS.boundaries);
...
export async function applyBoundaryUpdates(projectRoot: string, relativePath: string): Promise<{
  state: BoundaryState;
...
async function readStudyTargetBoundaries(projectRoot: string): Promise<Array<{ study_id: string; target_boundaries: string[] }>> {
  ...
  return Promise.all(
    studies.map(async (study) => {
      const record = await readMarkdownFrontmatter<StudyRecord>(projectRoot, `${PATHS.studiesDir}/${study.study_id}/study.md`);
      return {
...
export async function renderBoundaryGraphHtml(projectRoot: string, outputPath: string = PATHS.boundaryGraphHtml): Promise<string> {
  ...
  const studies = await discoverStudies(projectRoot);
  const updatesByStudy = await Promise.all(
    studies.map(async (study) => {
      ...
      if (!(await FileSystemUtils.fileExists(absolutePath))) {
        return {
          study_id: study.study_id,
          updates: [] as BoundaryUpdateSummaryEntry[],
        };
      }
  ...
  const relativeOutputPath = normalizeProjectRelativePath(
    projectRoot,
  ...
  const edgePayload = state.boundaries.flatMap((boundary) =>
    boundary.depends_on.map((dependencyId) => ({
      from: dependencyId,
      to: boundary.id,
    }))
  );
