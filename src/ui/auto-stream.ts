...
function resolveLocale(value?: AutoConsoleLocale): AutoConsoleLocale {
...
function maxIterationsLabel(value: number | null): string {
  return value === null ? 'unlimited' : String(value);
}
...
export function truncateAutoLogBlockForTest(value: string, limit = LOG_BLOCK_CHAR_LIMIT): string {
...
export class AutoConsoleRenderer {
  ...
  finish(result: AutoResult): void {
    ...
    this.line('');
    if (this.phaseCount > 0) this.separator();
  ...
  private describeToolStart(tool: AgentToolCall): string {
    if (tool.name === 'bash') {
  ...
  private toolStart(tool: AgentToolCall): void {
