...
export async function boundariesCommand(options: { json?: boolean } = {}): Promise<void> {
...
export async function boundariesApplyCommand(file: string | undefined, options: { json?: boolean } = {}): Promise<void> {
  if (!file?.trim()) {
...
export async function boundariesRenderCommand(options: { output?: string; json?: boolean } = {}): Promise<void> {
  const projectRoot = resolveProjectRoot();
  await requireQddProjectRoot(projectRoot);
...
function parseTargetIds(rawTargets: string | undefined): string[] {
  if (!rawTargets?.trim()) {
    return [];
  }
...
export async function boundariesScoreCommand(options: { targets?: string; study?: string; json?: boolean } = {}): Promise<void> {
