...
export interface RenderStoryCommandOptions {
...
export async function renderStoryCommand(storyPath: string, options: RenderStoryCommandOptions = {}): Promise<void> {
  ...
  if (report.pdf_path) console.log(`PDF: ${path.resolve(projectRoot, report.pdf_path)}`);
}
