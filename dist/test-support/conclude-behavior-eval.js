...
async function fetchJson(url) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`Literature service HTTP ${response.status}`);
...
async function searchCrossref(query) {
    ...
    if (items.length === 0)
        return `Crossref returned no records for: ${query}`;
    return items.map((item) => {
        const authors = Array.isArray(item.author)
            ? item.author.map((author) => [author.given, author.family].filter(Boolean).join(' ')).join(', ')
            : '';
        const published = Array.isArray(item.published?.['date-parts'])
            ? item.published['date-parts'][0]?.join('-')
            : '';
        return [`Title: ${item.title?.[0] ?? ''}`, `Authors: ${authors}`, `Venue: ${String(item['container-title'] instanceof Array ? item['container-title'][0] : '')}`, `Published: ${published}`, `DOI: ${String(item.DOI ?? '')}`].join('\n');
    }).join('\n\n');
}
async function searchGeo(accession) {
    const normalized = accession.trim().toUpperCase();
    if (!/^GSE\d+$/.test(normalized))
        throw new Error(`Invalid GEO series accession: ${accession}`);
    const searchUrl = new URL('https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi');
        ...
        try {
            switch (toolUse.name) {
                ...
                case 'search_crossref': {
                    const query = String(toolUse.input.query ?? '').trim();
                    if (!query)
                        throw new Error('Crossref query is required.');
