...
export const CONCLUDE_EVAL_FIXTURE_PATH = path.join(packageRoot, 'src', 'test', 'fixtures', 'conclude', 'sdk-two-gate');
...
class EvalConversation {
    ...
    stage = 'synthesis';
    sequence = 0;
    constructor(projectRoot, model, systemPrompt, secrets) {
...
async function prepareEvalProject(outputRoot) {
...
async function snapshotStory(projectRoot, outputRoot) {
...
async function scanGeneratedTextForSecrets(root, explicitSecrets) {
    const violations = [];
...
function evaluateAssertions(conversation, installedSkill, synthesisExists, storyExists, storyBeforeRevision, storyAfterRevision, secretViolations) {
    ...
    return [
        ...
        assertion('unpromoted_finalized_output_read', accesses.some((entry) => entry.action === 'read' && entry.path === UNPROMOTED_REPORT_PATH), UNPROMOTED_REPORT_PATH),
        assertion('figure_inspected_multimodally', accesses.some((entry) => entry.action === 'view_image' && entry.path === REQUIRED_FIGURE_PATH), REQUIRED_FIGURE_PATH),
...
function reportPaths(outputRoot, projectRoot) {
    return {
        run_root: outputRoot,
...
async function writeReportArtifacts(report, conversation) {
...
export async function runConcludeBehaviorEval(options) {
    ...
    const assertions = environmentBlocker
        ...
        : [
            ...
            ...evaluateAssertions(conversation, installedSkill, synthesisExists, storyExists, storyBeforeRevision, storyAfterRevision, secretViolations),
        ];
