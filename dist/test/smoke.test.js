...
async function withTempCodexHome(prefix, callback) {
...
function assertConcludeSkillContent(content) {
...
test('qdd-conclude ships a minimal Nature manuscript template', async () => {
    ...
    assert.deepEqual([...mainTemplate.matchAll(/\\section\{([^}]+)\}/g)].map((match) => match[1]), ['Introduction', 'Results', 'Discussion', 'Methods']);
...
async function readBootstrapManifest(projectRoot) {
...
test('qdd init installs qdd-conclude for Claude only with command, skill, and manifest records', async () => {
    ...
    assert.deepEqual(manifest.tools[0]?.assets.filter((entry) => entry.workflow === 'qdd-conclude').map((entry) => entry.path), ['.claude/commands/qdd-conclude.md', '.claude/skills/qdd-conclude/SKILL.md']);
});
test('qdd init installs qdd-conclude for Codex only with prompt, project skill, and manifest records', async () => {
    await withTempCodexHome('qdd-conclude-codex-home-', async (codexHome) => {
        ...
        assert.deepEqual(manifest.tools[0]?.assets.filter((entry) => entry.workflow === 'qdd-conclude').map((entry) => entry.path), [promptPath, '.codex/skills/qdd/qdd-conclude/SKILL.md']);
    });
});
