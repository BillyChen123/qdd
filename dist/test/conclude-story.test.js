...
test('story planner excludes an unsupported causal boundary from the claim graph', () => {
    ...
    const dossier = makeDossier([...resultUnits, unsupportedBoundary], [makeAsset('DOS-ASSET-0001', 'results/astrocyte-state.csv', resultUnits.map((unit) => unit.id))]);
    const plan = buildConcludeStoryPlan(dossier);
    assert.equal(plan.status, 'insufficient-story-diversity');
    assert.equal(plan.audit.status, 'pass');
    assert.equal(plan.candidates.length, 1);
...
test('story planner creates machine-verifiably distinct scientific claim routes', () => {
    ...
    const units = groups.flatMap((group, groupIndex) => group.statements.map((statement, statementIndex) => makeUnit({
        id: `DOS-EV-${String(groupIndex * 2 + statementIndex + 1).padStart(4, '0')}`,
        statement,
        sourcePath: group.sourcePath,
        assetId: group.assetId,
    })));
    const assets = groups.map((group) => makeAsset(group.assetId, group.sourcePath, units.filter((unit) => unit.provenance.sources[0]?.locator.path === group.sourcePath).map((unit) => unit.id)));
