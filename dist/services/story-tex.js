...
function parseMarkdown(story) {
    return unified().use(remarkParse).use(remarkGfm).parse(story);
}
function collectTextAnchors(value, citations, references) {
    let remaining = value;
...
async function analyzeStory(story, storyPath, projectRoot) {
    const root = parseMarkdown(story);
    const children = root.children ?? [];
    const significant = children.filter((node) => node.type !== 'yaml');
    if (significant.length === 0)
        throw new Error('story.md is empty.');
    const titleNodeIndex = children.findIndex((node) => node.type === 'heading' && node.depth === 1);
    if (titleNodeIndex < 0)
        throw new Error('story.md must contain one H1 title.');
    if (titleNodeIndex !== 0)
        throw new Error('story.md H1 title must be the first Markdown block.');
    ...
    if (!title)
        throw new Error('story.md H1 title must not be empty.');
    const abstractHeadingIndex = children.findIndex((node) => node.type === 'heading' && node.depth === 2 && normalizeText(nodeText(node)).toLowerCase() === 'abstract');
    if (abstractHeadingIndex < 0)
        throw new Error('story.md must contain a `## Abstract` section.');
    const citations = new Set();
    const references = new Set();
    const figures = [];
    const tables = [];
    const labels = new Set();
    const tableCaptionNodes = new Set();
    const sectionOrder = [];
    for (let index = 0; index < children.length; index += 1) {
        const node = children[index];
        walkTextNodes(node, (value) => collectTextAnchors(value, citations, references));
        if (node.type === 'heading')
            sectionOrder.push(normalizeText(nodeText(node)));
        if (node.type === 'paragraph' && node.children?.[0]?.type === 'image') {
            ...
            if (!LABEL_PATTERN.test(label) || labels.has(label))
                throw new Error(`Duplicate or invalid label: ${label}`);
            labels.add(label);
        ...
        if (node.type === 'paragraph') {
            ...
            if (match) {
                ...
                if (!next || next.type !== 'table')
                    throw new Error(`Table caption ${match[2]} must immediately precede a GFM table.`);
                const label = match[2];
                if (labels.has(label))
                    throw new Error(`Duplicate or invalid label: ${label}`);
                labels.add(label);
    ...
    for (const reference of references) {
        if (!labels.has(reference))
            throw new Error(`Broken story cross-reference: @${reference}`);
    }
    return {
        root,
        title,
...
function escapeTex(value) {
    return [...value].map((character) => {
        if (UNICODE_TEX[character])
            return UNICODE_TEX[character];
        switch (character) {
...
function renderText(value, context) {
    ...
    while (index < value.length) {
        ...
        if (remaining.startsWith('$$')) {
            ...
            if (end < 0)
                throw new Error('Unclosed display math delimiter in story.md.');
        ...
        if (remaining.startsWith('$')) {
            ...
            if (end < 0)
                throw new Error('Unclosed inline math delimiter in story.md.');
...
function renderTable(node, spec, context) {
    const rows = node.children ?? [];
    if (rows.length === 0)
        throw new Error(`Table ${spec.label} has no rows.`);
...
function renderBlock(node, index, context) {
    ...
    if (table)
        return renderTable(node, table, context);
    switch (node.type) {
...
function renderMainTex(analysis) {
    ...
    for (let index = 0; index < children.length; index += 1) {
        ...
        body.push(sourceMarker(block));
        if (context.tableCaptionNodes.has(index))
            continue;
...
function parseBibliography(input) {
    ...
    try {
        ...
        throw new Error(`Malformed BibTeX: ${error.message}`);
    }
    const keys = cite.data.map((entry) => String(entry['citation-key'] ?? entry.id ?? ''));
    if (keys.some((key) => !key))
        throw new Error('Malformed BibTeX entry without a citation key.');
    const duplicates = keys.filter((key, index) => keys.indexOf(key) !== index);
    if (duplicates.length > 0)
        throw new Error(`Duplicate BibTeX keys: ${[...new Set(duplicates)].join(', ')}`);
    return cite.data;
}
...
function ppmToPng(source) {
    const tokens = source.split(/\r?\n/)
        ...
        .filter(Boolean);
    if (tokens[0] !== 'P3')
        throw new Error('Only ASCII P3 PPM figure assets are supported.');
...
async function copyFigure(figure, destination) {
...
function selectBibliography(input, citationKeys) {
    ...
    if (missing.length > 0)
        throw new Error(`Missing verified BibTeX entries for story citations: ${missing.join(', ')}`);
...
function validateBalancedTex(tex) {
    ...
    for (let index = 0; index < content.length; index += 1) {
        ...
        if (character === '}')
            depth -= 1;
        if (depth < 0)
            throw new Error('TeX syntax check failed: unmatched closing brace.');
    }
    if (depth !== 0)
        throw new Error('TeX syntax check failed: unmatched opening brace.');
    const environments = [];
    ...
    if (environments.length > 0)
        throw new Error(`TeX syntax check failed: unclosed ${environments.at(-1)} environment.`);
}
async function validatePackage(stageDir, tex, analysis, renderedBlocks, bibliographyKeys) {
    ...
    if ([...labels].sort().join('\0') !== expectedLabels.join('\0'))
        throw new Error('TeX label coverage check failed.');
    const refs = [...tex.matchAll(/\\ref\{([^}]+)\}/g)].map((match) => match[1]);
    const missingRefs = refs.filter((reference) => !labels.includes(reference));
    if (missingRefs.length > 0)
        throw new Error(`Broken TeX references: ${missingRefs.join(', ')}`);
    ...
    if (missingCitations.length > 0)
        throw new Error(`Broken TeX citations: ${missingCitations.join(', ')}`);
    ...
    if (graphics.join('\0') !== expectedGraphics.join('\0'))
        throw new Error('Story/TeX figure coverage or order check failed.');
    for (const graphic of graphics)
        await fs.access(path.join(stageDir, graphic));
    ...
    if (actualMarkers.join('\0') !== expectedMarkers.join('\0'))
        throw new Error('Story/TeX coverage or order check failed.');
    if (analysis.blocks.length !== renderedBlocks.length)
        throw new Error('Story/TeX block count mismatch.');
}
...
export function probeTexCompiler(hasBibliography = true) {
    const latexmk = findExecutable('latexmk');
    const bibtex = findExecutable('bibtex');
    if (latexmk && (!hasBibliography || bibtex))
    ...
    if (pdflatex && (!hasBibliography || bibtex))
        return { kind: 'pdflatex', command: pdflatex, bibtexCommand: bibtex ?? undefined };
    return null;
}
async function compilePdf(stageDir, compiler, hasBibliography) {
...
async function replaceDirectory(stageDir, outputDir) {
...
export async function renderAcceptedStory(options) {
    ...
    if (analysis.citations.length > 0) {
        if (!options.bibliographyPath)
            throw new Error('A verified --bibliography file is required because story.md contains citation anchors.');
    ...
    try {
        ...
        if (compiler)
            await compilePdf(stageDir, compiler, analysis.citations.length > 0);
        const report = {
            schema_version: 1,
...
//# sourceMappingURL=story-tex.js.map