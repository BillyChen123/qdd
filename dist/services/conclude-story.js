...
function claimTokens(value) {
    return new Set(normalizeComparison(value)
        .split(' ')
...
function isNarrativeEligible(unit) {
...
function isBoundaryEligible(unit) {
    const statement = claimStatement(unit);
...
function unitScore(unit, assetById) {
    const statement = claimStatement(unit);
...
function deduplicateUnits(units, assetById) {
...
function buildClaimGroups(units, assetById) {
...
function selectRouteUnits(primary, groups, assetById) {
...
function selectBoundaryUnits(resultUnits, boundaries) {
...
function buildFigureTablePlan(resultUnits, assetById) {
    const plan = [];
    const seen = new Set();
...
function buildMissingValidation(resultUnits, boundaryUnits) {
    const uncertainty = resultUnits.flatMap((unit) => unit.narrative.uncertainty.map((value) => `[${unit.id}] ${normalizeText(value)}`));
...
function buildReviewerRisk(resultUnits, boundaryUnits, figurePlan) {
    const risks = [
...
function buildClaimGraph(resultUnits, boundaryUnits, figureTablePlan, missingValidation, reviewerRisk) {
    const lead = resultUnits[0];
    const resultNodes = resultUnits.map((unit, index) => ({
        claimId: unit.id,
    ...
    const boundaryNodes = boundaryUnits.map((unit) => ({
        claimId: unit.id,
    ...
    const edges = [
        ...
        ...boundaryNodes.map((node) => ({
            ...
            toClaimId: lead.id,
            relation: node.role === 'contradiction' ? 'contradicts' : 'bounds',
...
function buildViabilityDiagnostics(resultUnits, availableResultClaims, boundaryUnits, figurePlan, missingValidation) {
...
function buildCandidate(candidateIndex, resultUnits, boundaryUnits, allResultUnits, assetById, dossierAuditPassed) {
    const lead = resultUnits[0];
    ...
    const resultsArc = resultUnits.map((unit, index) => ({
        sequence: index + 1,
        claimId: unit.id,
        statement: claimStatement(unit),
        role: index === 0 ? 'lead' : 'supporting',
    ...
    const limitationPlacement = boundaryUnits.map((unit, index) => ({
        ...
        placement: index === 0 ? 'results' : 'discussion',
    ...
    return {
        ...
        limitationPlacement,
        viabilityBlockers,
        viability: buildViabilityDiagnostics(resultUnits, allResultUnits.length, boundaryUnits, figureTableSequence, missingValidation),
        ...
        claimsAllowed: resultUnits.map((unit) => `[${unit.id}] Use ${unit.narrative.allowedVerbs.join(', ') || 'bounded descriptive wording'} for this claim.`),
        claimSafetyLimits: claimGraph.claimSafety,
        claimsToSoftenOrAvoid: resultUnits.map((unit) => `[${unit.id}] Avoid ${unit.narrative.forbiddenVerbs.join(', ') || 'language stronger than the recorded claim strength'}.`),
...
function candidatesAreDistinct(left, right) {
...
function collectNarrativeFields(candidate) {
    return [
...
export function auditConcludeStoryPlan(candidates) {
    const violations = [];
...
export function buildConcludeStoryPlan(dossier) {
    ...
    for (const group of groups) {
        ...
        const candidate = buildCandidate(candidates.length + 1, routeUnits, matchedBoundaries, resultUnits, assetById, dossier.audit.status === 'pass');
