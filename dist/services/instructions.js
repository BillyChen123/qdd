...
async function readStudyRecord(projectRoot, studyId) {
    return readMarkdownFrontmatter(projectRoot, `studies/${studyId}/study.md`);
}
async function findTaskRecord(projectRoot, taskId) {
    ...
    for (const studyEntry of studies) {
        ...
        if (await FileSystemUtils.fileExists(taskPath)) {
            const task = await readMarkdownFrontmatter(projectRoot, `studies/${studyEntry.name}/tasks/${taskId}.md`);
            return {
                task: {
                    ...task,
...
async function collectContextReadPaths(projectRoot, relativeDir = PATHS.contextDir) {
...
async function collectSharedDataReadPaths(projectRoot) {
...
async function collectStudyTaskPaths(projectRoot, studyId) {
...
async function collectTaskSkillIds(projectRoot, taskPaths) {
    const skillIds = new Set();
    for (const relativePath of taskPaths) {
        const task = await readMarkdownFrontmatter(projectRoot, relativePath);
...
function allowedPlanningCategoriesForRole(role) {
    switch (role) {
        case 'thesis-manager':
            return ['thesis'];
        case 'study-brain':
...
function appendRoleSkillIssues(rules, subject, roleSkills) {
...
function appendArtifactCandidateSchemaRules(rules) {
    ...
    rules.push('Minimal artifact-candidates.yaml shape: artifact_candidates: [{ path: studies/STUDY-XXX/output/tables/example.csv, type: table, task_id: TASK-XXX, reusable: true, scope: study, description: "Short reason this output is worth promoting.", schema: csv-table }].');
}
function appendStudyStatusSchemaRules(rules) {
...
function appendResourceMemoryRules(rules) {
...
async function resolveRoleSkillSet(projectRoot, role, policy) {
...
export async function buildInstructions(projectRoot, id, options = {}) {
