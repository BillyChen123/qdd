...
function extractVisibleManuscriptText(mainTex) {
    return normalizeLine(stripLatexComments(mainTex)
        .replace(/\\[a-zA-Z*]+\[[^\]]*\]\{([^}]*)\}/g, ' $1 ')
        .replace(/\\[a-zA-Z*]+\{([^}]*)\}/g, ' $1 ')
        .replace(/\\[a-zA-Z*]+/g, ' ')
        .replace(/[{}]/g, ' ')
...
function collectVisibleCausalOverclaims(visibleText) {
...
function scoreLogicalCoherence(mainTex, claims, visibleText) {
    ...
    return buildDimensionScore('logical_coherence', score, sectionCount >= 4 && prosePenalty === 0
...
function scoreManuscriptViability(run, mainTex, visibleText) {
    ...
    return buildDimensionScore('manuscript_viability', score, /\\bibliography\{references\}/.test(mainTex) && reportTonePenalty === 0
...
export async function runConcludeEval(options) {
    ...
    const hardFails = [
        ...
        buildHardFail('associative_to_causal_overclaim', associativeToCausalOverclaims.length > 0 || visibleCausalOverclaims.length > 0, 'Associative evidence must not be written as a proven causal mechanism.', [...associativeToCausalOverclaims, ...visibleCausalOverclaims]),
        buildHardFail('raw_task_study_leakage', rawTaskStudyLeakage.length > 0, 'Visible manuscript prose must not leak raw task/study execution markers or audit headings.', rawTaskStudyLeakage),
        buildHardFail('report_tone_dominates_manuscript', reportToneSignals.length >= 2, 'Visible manuscript prose must read like a paper draft rather than instructions, audit scaffolding, or report-tone meta commentary.', reportToneSignals),
    ];
