...
function buildAbstractParagraph(candidate, claims) {
    ...
    return sentenceCaseTrim(`${candidate.centralClaim} We summarize the selected QDD storyline as a submission-oriented draft grounded in internal evidence. The current abstract remains intentionally conservative: ${leadClaim} Negative and blocked studies stay visible to explain why stronger mechanistic language is not used.`);
}
function buildIntroductionParagraph(candidate, hasExternalCitations) {
    ...
    return sentenceCaseTrim(`${candidate.story} The introduction should motivate the biological or methodological question, explain why the QDD evidence package matters, and clearly separate internal results from field context. ${citationSuffix}`);
}
function buildDiscussionParagraph(candidate, claims) {
    const boundarySummary = uniqueStrings(claims.flatMap((claim) => claim.boundaryEvidence.map((item) => item.summary))).slice(0, 3);
    return sentenceCaseTrim(`The discussion should explain how the selected story remains bounded by the available evidence, surface reviewer-facing limitations, and preserve negative evidence as a strength rather than a hidden weakness. ${boundarySummary.length > 0 ? `Key boundary evidence includes ${boundarySummary.join('; ')}.` : 'Boundary evidence should be expanded as the draft matures.'}`);
}
