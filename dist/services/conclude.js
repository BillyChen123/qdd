...
function stripReportTone(text) {
    return sentenceCaseTrim(stripExecutionLeakage(text)
...
function shortenSummary(text, maxLength = 240) {
...
function softenCausalLanguage(text) {
    return sentenceCaseTrim(text
...
function toTexRelativePath(value) {
...
function isStructuredStoryCandidate(value) {
...
function parseSelectedStoryCandidate(content) {
...
function buildResultsClaims(candidate, dossier) {
    const unitById = new Map(dossier.evidenceUnits.map((unit) => [unit.id, unit]));
...
function buildFinalPaperArtifactPaths(artifactPaths) {
...
function buildFinalArtifactStatus(status, pathValue, details, notes = []) {
    return {
        status,
        path: pathValue,
        details,
...
async function collectCitationEntries(artifactPaths) {
...
async function materializeFigureAssets(projectRoot, claims, finalPaths) {
...
function renderFigureAssetMapMarkdown(figureAssets) {
    const lines = [
        '# Figure Asset Map',
        '',
    ...
    for (const asset of figureAssets) {
        lines.push(`| ${asset.label} | ${asset.resultClaimId} | ${asset.evidenceId ?? 'n/a'} | ${asset.sourcePath ?? 'n/a'} | ${asset.targetPath ?? 'placeholder'} | ${asset.status.toUpperCase()} | ${asset.recommendedUse} | ${asset.notes.join(' ') || 'n/a'} |`);
    }
    lines.push('');
    return lines.join('\n');
}
function renderReferencesBib(citationEntries) {
...
function buildAbstractParagraph(candidate, claims) {
    ...
    const boundarySummary = uniqueStrings(claims.flatMap((claim) => claim.boundaryEvidence.map((item) => firstNarrativeSentence(removeLeadingCue(item.summary))))).slice(0, 2);
...
function buildIntroductionParagraph(candidate, hasExternalCitations) {
    ...
    return sentenceCaseTrim(`${opening} The introduction frames the biological or methodological question, explains why the available evidence is synthesis-ready, and keeps field context distinct from project-derived results. ${citationSuffix}`);
}
function buildDiscussionParagraph(candidate, claims) {
    const boundarySummary = uniqueStrings(claims.flatMap((claim) => claim.boundaryEvidence.map((item) => shortenSummary(removeLeadingCue(item.summary), 140)))).slice(0, 3);
    const safetySummary = uniqueStrings(claims.flatMap((claim) => claim.claimSafetyNotes)
        .map((note) => note.replace(/^[A-Z]+:\s*/i, ''))
        .map((note) => shortenSummary(note, 120))).slice(0, 2);
    return sentenceCaseTrim([
...
function buildResultsOpeningParagraph(claims, figureAssets) {
...
function buildResultsNarrativeParagraph(claim) {
    const supportSummary = uniqueStrings(claim.supportingEvidence.map((item) => firstNarrativeSentence(removeLeadingCue(item.summary)))).slice(0, 3);
    const boundarySummary = uniqueStrings(claim.boundaryEvidence.map((item) => firstNarrativeSentence(removeLeadingCue(item.summary)))).slice(0, 2);
...
function buildBoundaryInterpretationParagraph(claim) {
    if (claim.boundaryEvidence.length === 0) {
        return null;
    }
    const boundarySummary = uniqueStrings(claim.boundaryEvidence.map((item) => firstNarrativeSentence(removeLeadingCue(item.summary)))).slice(0, 2);
    ...
    return sentenceCaseTrim(`Rather than weakening the manuscript, these boundary observations clarify the present scope of inference: ${lowerCaseLead(boundarySummary.join('; '))}.`);
}
function renderManuscriptEvidenceTraceComment(claim) {
    const lines = [
...
function determineOverallFinalStatus(statuses) {
...
async function tryRenderPdf(finalPaths, preflight) {
    if (preflight.render.pdf.status !== 'available') {
        return buildFinalArtifactStatus('blocked', null, 'PDF rendering is blocked because the local TeX toolchain is incomplete.', preflight.render.pdf.reasons);
    }
    ...
    if (execution.exitCode !== 0 || !(await FileSystemUtils.fileExists(finalPaths.paperPdfPath))) {
        return buildFinalArtifactStatus('blocked', null, `PDF rendering failed when running ${command}.`, [`See \`${path.basename(finalPaths.pdfRenderLogPath)}\` for details.`]);
    }
    return buildFinalArtifactStatus('complete', finalPaths.paperPdfPath, `PDF rendering succeeded via ${command}.`, [`Render log: \`${path.basename(finalPaths.pdfRenderLogPath)}\`.`]);
}
async function tryRenderWord(finalPaths, preflight) {
    if (preflight.render.word.status !== 'available') {
        return buildFinalArtifactStatus('blocked', null, 'Word rendering is blocked because pandoc is unavailable.', preflight.render.word.reasons);
    }
    ...
    if (execution.exitCode !== 0 || !(await FileSystemUtils.fileExists(finalPaths.paperDocxPath))) {
        return buildFinalArtifactStatus('blocked', null, 'Word rendering failed when running pandoc.', [`See \`${path.basename(finalPaths.wordRenderLogPath)}\` for details.`]);
    }
    return buildFinalArtifactStatus('complete', finalPaths.paperDocxPath, 'Word rendering succeeded via pandoc.', [`Render log: \`${path.basename(finalPaths.wordRenderLogPath)}\`.`]);
}
function renderFinalArtifactAuditMarkdown(finalPaper) {
    const lines = [
...
async function generateFinalPaperPackage(projectRoot, candidate, claims, artifactPaths, preflight) {
    ...
    const mainTex = buildFinalArtifactStatus('complete', finalPaths.mainTexPath, 'Generated a reviewable TeX manuscript skeleton with Abstract, Introduction, Results, and Discussion.', claims.length > 0
    ...
    const referencesBib = citationEntries.length > 0
        ? buildFinalArtifactStatus('complete', finalPaths.referencesBibPath, `Generated references.bib with ${citationEntries.length} verified BibTeX entr${citationEntries.length === 1 ? 'y' : 'ies'}.`, ['Only verified BibTeX entries were emitted; no placeholder citations were fabricated.'])
        : buildFinalArtifactStatus('gap', finalPaths.referencesBibPath, 'Generated an empty bibliography scaffold because no verified BibTeX entries were available.', citationGaps);
    const figures = figureAssets.some((asset) => asset.status === 'placeholder')
        ? buildFinalArtifactStatus('gap', finalPaths.figureAssetMapPath, 'Generated a figure asset map, but one or more Results claims still rely on placeholders.', ['Review `figures/asset_map.md` before submission packaging.'])
        : buildFinalArtifactStatus('complete', finalPaths.figureAssetMapPath, 'Generated figure asset map with reusable internal evidence assets.', [`Mapped ${figureAssets.length} figure asset entr${figureAssets.length === 1 ? 'y' : 'ies'}.`]);
    const citationIntegrity = citationGaps.length > 0
        ? buildFinalArtifactStatus('gap', finalPaths.referencesBibPath, 'Citation integrity is incomplete because verified external bibliography support is missing.', citationGaps)
        : buildFinalArtifactStatus('complete', finalPaths.referencesBibPath, 'Citation integrity is currently satisfied for external statements that use verified BibTeX support.', ['Results claims remain anchored to internal QDD evidence only.']);
...
async function resolveSelectedStory(projectRoot, outputDir, options, candidates, dossier) {
...
function renderStoryPlanMarkdown(result) {
    const lines = [
...
export async function runConclude(projectRoot, options = {}) {
    ...
    const finalPaperArtifacts = baseResult.selectedCandidate && baseResult.planningArtifacts
        ? await generateFinalPaperPackage(preflight.projectRoot, baseResult.selectedCandidate, baseResult.resultsClaims, baseResult.planningArtifacts, preflight)
        : null;
...
export function renderConcludeRenderStatusMarkdown(result, finalPaper = null) {
