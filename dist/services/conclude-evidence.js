...
function parseProducedBy(value) {
...
async function buildSourceKey(projectRoot, relativePath) {
    try {
...
function mergeSource(target, incoming) {
    ...
    return {
        ...
        registrations: unique([...target.registrations, ...incoming.registrations]),
...
async function sourceFromIndex(projectRoot, artifact) {
...
async function sourceFromCandidate(projectRoot, studyId, candidate) {
...
async function collectArtifactSources(result) {
...
async function readBoundedText(absolutePath, maxBytes = CONCLUDE_EVIDENCE_READ_LIMITS.maxBytesPerTextSource) {
...
function buildRowSelector(record, headers) {
    const selectorHeaders = headers.filter((header) => /^(?:gene_symbol|gene|symbol|feature_id|transcript_id|transcript|term|pathway|dimension|subcluster|cluster|population|comparison|condition)$/i.test(header));
...
function scoreTableRow(record, narrative) {
...
function makeSourceRef(source, role, sourceType, locator) {
    return {
        role,
        sourceType,
        locator,
...
function emptyNarrative(claimStrength = 'bounded') {
    return {
...
function buildTableUnits(source, read, delimiter) {
...
function buildContextUnit(source, reason, read, format) {
    return {
        id: '',
...
function cleanMarkdownNarrative(value) {
...
function extractTextStatistics(text) {
    const statistics = [];
...
function inferTextPopulation(text) {
...
function inferTextComparison(text) {
...
function buildMarkdownNarrative(statement, eligibility) {
...
function buildMarkdownUnits(source, read) {
    const units = [];
    for (const paragraph of parseMarkdownParagraphs(read.content)) {
        ...
        const eligibility = isBoundary ? 'boundary' : 'results';
        const role = isBoundary
...
function safeCatalogCaption(descriptions) {
...
async function readFigureDimensions(absolutePath, format) {
    const handle = await fs.open(absolutePath, 'r');
    try {
        const stats = await handle.stat();
...
function buildTableAsset(source) {
...
async function buildFigureAsset(projectRoot, source) {
...
async function buildExcludedUnit(projectRoot, source, reason) {
    let bytesRead = 0;
    try {
...
function collectNarrativeFields(unit) {
...
export function auditConcludeEvidenceDossier(dossier) {
    const violations = [];
    for (const unit of dossier.evidenceUnits) {
        ...
        const hasVerifiableLocator = unit.provenance.sources.some((source) => source.locator.kind === 'table-row' || source.locator.kind === 'markdown-lines');
...
export async function buildConcludeEvidenceDossier(result, now = new Date()) {
    ...
    for (const source of sources) {
        ...
        try {
            ...
            const reason = `Source could not be read within the dossier contract: ${error.message}`;
            const excluded = await buildExcludedUnit(result.projectRoot, source, reason);
            evidenceUnits.push(excluded.unit);
            gaps.push(excluded.gap);
        }
...
export function renderConcludeEvidenceDossierMarkdown(dossier) {
    const lines = [
        '# Evidence Dossier',
        '',
...
//# sourceMappingURL=conclude-evidence.js.map