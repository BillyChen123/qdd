...
export function listManagedFileContracts() {
    return [...MANAGED_FILE_CONTRACTS];
}
export function getManagedFileContract(id) {
...
export function getManagedFileExamplePath(contract) {
    return `${PATHS.examplesDir}/${contract.exampleFileName}`;
}
export function buildManagedFileReferenceOutputs() {
    const outputs = [
        {
    ...
    return outputs.concat(MANAGED_FILE_CONTRACTS.map((contract) => ({
        relativePath: getManagedFileExamplePath(contract),
        content: contract.renderExample(),
    })));
}
export function listManagedFileReferencePaths() {
    return buildManagedFileReferenceOutputs().map((entry) => entry.relativePath);
}
export function listManagedFileReferencePathsForTarget(target) {
