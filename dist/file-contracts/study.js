...
export const STUDY_STATUS_VALUES = ['created', 'confirmed', 'running', 'blocked', 'completed', 'closed'];
export function renderStudyBody(record) {
    const evidencePlan = renderBulletList(record.expected_artifacts ?? [], '- Define the minimum evidence needed to judge this study.');
    return [
        '## Question',
        '',
        record.question,
        ...
        '## Target Boundaries',
        '',
        renderBulletList(record.target_boundaries ?? [], '- None declared yet. Read `evolution.yaml` and declare only the current open boundaries this study will actually clarify.'),
        '',
        '## Why Now',
        '',
...
export function renderStudyMarkdown(record) {
    return renderMarkdownDocument(record, renderStudyBody(record));
}
export function createExampleStudyRecord() {
    return {
        study_id: 'STUDY-001',
...
export const studyFileContract = {
    id: 'study',
