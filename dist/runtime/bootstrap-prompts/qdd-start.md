...
When running in auto mode, after you have completed all start-mode work above and the project is ready for its first study:

1. Confirm the filesystem state is consistent — `contract.yaml`, `context/resources.md`, and any dataset entrypoints are written.
2. Fork a Study Brain sub-agent to create the first study:

   ```
   /fork Study Brain for STUDY-001 with command qdd-propose.

   Context to pass:
   - The project root directory: <current working directory>
   - Run: qdd instructions STUDY-001 --command qdd-propose --json
