...
- Write Methods only from details present in inspected sources. Do not infer sample structure, collection procedures, assay names, computational methods, statistical tests, controls, regeneration conditions, or data availability from a result table or project status. When the available record supports only a generic phrase such as "operando measurements" or "static screening," preserve that level of specificity and state which acquisition details remain undocumented instead of filling them with a plausible method.
