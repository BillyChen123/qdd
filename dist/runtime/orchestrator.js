...
function extractTextBetween(content, startMarker, endMarker) {
...
function parseCurrentStudyId(status) {
    const active = status.studies.active;
    if (active.length > 0)
        return active[active.length - 1];
    const blocked = status.studies.blocked;
    if (blocked.length > 0)
        return blocked[blocked.length - 1];
    return null;
}
function determineNextStudyId(status) {
...
function checkTermination(status) {
...
function computeInitialPhase(status) {
    const allStudies = [
        ...status.studies.active,
        ...status.studies.blocked,
        ...status.studies.completed,
...
function nextPhase(currentPhase, status) {
    switch (currentPhase) {
        case 'start':
            return {
                phase: 'propose',
